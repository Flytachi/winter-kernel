 ...
 * use.
 *
 * It exists so {@see PpaConnectionPool::reportFailure()} and the `defer` that returns
 * the connection can agree without reading the coroutine context during teardown:
 * the defer captures this object directly, and reporting a failure flips {@see $dead}
 * so the connection is evicted instead of pushed back into the pool.
 */
